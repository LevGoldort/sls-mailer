...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <p><strong>🎫 Билеты:</strong> {% for ticket in order.tickets %}{{ ticket.quantity }}x {{ ticket.type_name }}{% if ticket.purchased_seats %} ({% for seat_id in ticket.purchased_seats %}Ряд {{ seat_display_map[seat_id].row }}, Место {{ seat_display_map[seat_id].seat }}{% if not loop.last %}; {% endif %}{% endfor %}){% endif %}{% if not loop.last %}, {% endif %}{% endfor %}</p>
            ...
            <div style="border: 2px solid #e0e0e0; border-radius: 8px; padding: 15px; margin-top: 15px;">
                ...
                <div style="display: table; width: 100%;">
                    <div style="display: table-cell; vertical-align: top; width: 60%;">
                        ...
                        {% if qr['seat_id'] %}
                        <p style="margin: 0 0 6px 0; font-size: 15px; font-weight: 600; color: #667eea;">💺 Ряд {{ qr['display_row'] }}, Место {{ qr['display_seat'] }}</p>
                        {% endif %}
    ...
    try:
        ...
        try:
            ...
            if location_data:
                ...
                # Extract seating map config for seat display numbers
                if location.venue_config and location.venue_config.seating_map:
                    seating_map_config = location.venue_config.seating_map
