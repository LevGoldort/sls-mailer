...
update_lambda() {
    ...
    # Create zip package (use absolute paths)
    local abs_build_dir="$SCRIPT_DIR/$build_dir"
    local zip_file="$SCRIPT_DIR/${function_name}.zip"
...
(cd "$SCRIPT_DIR/frontend" && zip -r -q "$SCRIPT_DIR/site-templates-layer.zip" templates static)
